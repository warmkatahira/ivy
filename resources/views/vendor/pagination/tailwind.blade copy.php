@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        <div class="flex justify-between sm:hidden xl:hidden">
            <!-- 前のページに移動する矢印 -->
                    ...
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <span aria-current="page">
                                <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-500 bg-teal-200 border border-gray-300 cursor-default leading-5">{{ $page }}</span>
                            </span>
                        @else
