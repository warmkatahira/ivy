...
var inventory_quantity_difference = document.getElementById("inventory_quantity_difference");
...
var today_inventory_quantity = document.getElementById("today_inventory_quantity");
      ...
      success: function success(data) {
        ...
        if (data['item_difference_flg'] == false && data['item_searched_flg'] == true) {
          ...
          if (data['inventory_quantity'] == 1) {
            ...
            logical_stock.innerHTML = data['item']['logical_stock']; // 商品情報を表示
          ...
          inventory_quantity_difference.innerHTML = Number(data['today_inventory_quantity']) + Number(data['inventory_quantity']) - Number(data['item']['logical_stock']); // 当日の累計棚卸数を表示
...
function difference_item_confirm() {
  ...
  $.ajax({
    ...
    type: 'GET',
    dataType: 'json',
    success: function success(data) {// ここでは何もしない（コントローラーで確定処理が行われている）
    },
    error: function error() {
      error_msg("通信に失敗しました");
    }
...
$("[id=inventory_modify]").on("click", function () {
  ...
  var modify_quantity = prompt('訂正数を入力して下さい。（減らす場合はマイナスを付けて下さい。）'); // 値がnullだったら、ここで処理を終了
  ...
  $.ajax({
    ...
    success: function success(data) {
      ...
      inventory_quantity.innerHTML = data['inventory_quantity']; // 棚卸差分を更新
